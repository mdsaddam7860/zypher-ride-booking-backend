...
export function serializeRide(
  ...
  const base = {
    ...
    waitingCharge: Number(ride.waiting_charge),
    createdAt: ride.created_at,
  };

  const fareAmount = opts?.fare
    ? {
        estimatedPrice: Number(opts.fare.estimated_price),
        currency: opts.fare.currency,
      }
    : undefined;
  ...
  const canSeeBilling =
    opts?.viewerRole === "rider" || opts?.viewerRole === "owner";
  ...
  return {
    ...
    billing: canSeeBilling
      ? {
          ...
          refundAmount:
            ride.refund_amount === null ? null : Number(ride.refund_amount),
        }
      : undefined,
  };
}
